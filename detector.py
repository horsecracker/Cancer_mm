...
class Detector():
    ...
    def get_classmap(self, label, conv6, imagesize):
        conv6_resized = tf.image.resize_bilinear( conv6, [imagesize, imagesize] )
        with tf.variable_scope("GAP", reuse=True):
            label_w = tf.gather(tf.transpose(tf.get_variable("W")), label)
            label_w = tf.reshape( label_w, [-1, 1024, 1] ) # [batch_size, 1024, 1]

        conv6_resized = tf.reshape(conv6_resized, [-1, imagesize*imagesize, 1024]) # [batch_size, 224*224, 1024]

        classmap = tf.batch_matmul( conv6_resized, label_w )
        classmap = tf.reshape( classmap, [-1, imagesize,imagesize] )
        return classmap
