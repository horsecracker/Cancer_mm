...
train_log='./tensorboard_out/density_train_log-augmented'
test_log='./tensorboard_out/density_test_log-augmented'
save_model_name='./density_dropout0.5_alllayers.ckpt'

logfile='./logfile/log_orignal_augmented.txt'
f=open(logfile, 'a+')
f.write('with augmented data; no sigmoid; dropout on fc1 \n')
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        ...
        if step % display_step == 1:
            ...
            print('epoch: %d' %((step*batch_size)/2663.0))
            print("Iter " + str(step*batch_size) + ", Minibatch Loss= " + \
                  "{:.6f}".format(loss) + ", Training Accuracy= " + \
                  "{:.5f}".format(acc))
	    #f=open(logfile, 'r+')
	    f.write('\n')
            f.write('epoch: %d \n' %((step*batch_size)/2663.0))
